...
ANIMAL_CONFIG: List[Tuple[str, int, str, str, str, int, str]] = [
    ...
    # ------------------- RARE -------------------
    ("dragons",     50, "🐉", "Драконы",           "rare",  5000,
        "Огнедышащие создания, приносящие огромный доход."),
    ("unicorns",    45, "🦄", "Единороги",         "rare",  4500,
        "Мифические существа, которые приносят удачу и богатство."),
    ("phoenixes",   40, "🦅", "Фениксы",           "rare",  4000,
        "Восстают из пепла, принося стабильный доход."),
]
...
async def admin_panel(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    total_users = _execute("SELECT COUNT(*) FROM users").fetchone()[0]
    active_users = _execute("SELECT COUNT(*) FROM users WHERE last_active > ?", (time.time() - 86400,)).fetchone()[0]

    # Обновление текста админ-панели
    caption = f"🔥 Админ‑панель 🔥\n\n👥 Всего пользователей: {total_users}\n🟢 Активных за последние 24 часа: {active_users}"
