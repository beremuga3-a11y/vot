...
def build_ticket_answer_caption(ticket_id: int, answer_text: str) -> str:
    """Формирует безопасный многострочный текст ответа на заявку.

    Используйте вместо прямого f-строки вида:
        f"💬 Ответ по вашей заявке #{ticket_id}:\n{answer_text}"
    чтобы избежать ошибок незавершённых строк и проблем с переносами.
    """
    return f"💬 Ответ по вашей заявке #{ticket_id}:\n{answer_text}"
