...
SECTION_IMAGES: Dict[str, str] = {
    ...
    "support": "https://i.postimg.cc/fb1TQF6W/5355070803995131046.jpg",
    "achievements": "https://i.postimg.cc/mg2rY7Y4/5355070803995131023.jpg",
}
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
#   Достижения: модель и утилиты ← NEW
# ----------------------------------------------------------------------
ACHIEVEMENTS_DEF: List[Tuple[str, str, str]] = [
    ("first_purchase", "Первое приобретение", "Купите любого питомца"),
    ("rich_1k", "Мелкий богач", "Накопите 1 000 монет"),
    ("rich_1m", "Магнат", "Накопите 1 000 000 монет"),
    ("first_feed", "Заботливый", "Покормите питомца"),
    ("first_farmer", "Работодатель", "Купите фермера"),
]


def init_achievements_table() -> None:
    _execute(
        """
...
def has_achievement(user_id: int, key: str) -> bool:
    cur.execute("SELECT 1 FROM achievements WHERE user_id = ? AND key = ?", (user_id, key))
    return cur.fetchone() is not None


def award_achievement(user_id: int, key: str) -> None:
    if not has_achievement(user_id, key):
        _execute(
            "INSERT INTO achievements (user_id, key, ts) VALUES (?,?,?)",
            (user_id, key, int(time.time())),
        )
        log_user_action(user_id, f"Получено достижение: {key}")


def list_achievements(user_id: int) -> Tuple[List[sqlite3.Row], List[Tuple[str, str, str]]]:
    cur.execute("SELECT key, ts FROM achievements WHERE user_id = ?", (user_id,))
    earned = {row["key"]: row["ts"] for row in cur.fetchall()}
    earned_list = [(k, t) for k, t in earned.items()]
    locked_defs = [rec for rec in ACHIEVEMENTS_DEF if rec[0] not in earned]
    return earned_list, locked_defs


def check_and_award_achievements(user_id: int) -> None:
    user = get_user(user_id)
    # Покупка питомца
    if not has_achievement(user_id, "first_purchase"):
        if any(user[f] > 0 for f, *_ in ANIMAL_CONFIG):
            award_achievement(user_id, "first_purchase")
    # Монеты
    if user["coins"] >= 1_000 and not has_achievement(user_id, "rich_1k"):
        award_achievement(user_id, "rich_1k")
    if user["coins"] >= 1_000_000 and not has_achievement(user_id, "rich_1m"):
        award_achievement(user_id, "rich_1m")
    # Фермер
    if not has_achievement(user_id, "first_farmer"):
        cur.execute("SELECT 1 FROM farmers WHERE user_id = ? AND qty > 0 LIMIT 1", (user_id,))
        if cur.fetchone() is not None:
            award_achievement(user_id, "first_farmer")
...
# Удалено: функционал клановых битв
...
async def feed_animal(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Достижение: первое кормление
    award_achievement(uid, "first_feed")
...
# ----------------------------------------------------------------------
#   Достижения (раздел) ← NEW
# ----------------------------------------------------------------------
async def achievements_section(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    uid = query.from_user.id
    check_and_award_achievements(uid)
    earned, locked_defs = list_achievements(uid)
    lines = ["🏆 Ваши достижения:\n"]
    if earned:
        for key, ts in earned:
            name = next((n for k, n, _ in ACHIEVEMENTS_DEF if k == key), key)
    ...
    if locked_defs:
        lines.append("\n🔒 Доступные цели:")
        for key, name, desc in locked_defs[:10]:
            lines.append(f"• {name}: {desc}")
    text = "\n".join(lines)
    await edit_section(
        query,
        caption=text,
        image_key="achievements",
        reply_markup=InlineKeyboardMarkup(
...
#   Техподдержка ← NEW
# ----------------------------------------------------------------------
async def support_section(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    context.user_data["awaiting_support_message"] = True
    await edit_section(
        query,
        caption=(
            "🆘 Техподдержка\n\n"
            "Опишите вашу проблему одним сообщением. Мы увидим его и ответим."
        ),
        image_key="support",
        reply_markup=InlineKeyboardMarkup(
...
async def buy_confirm(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Достижения (первая покупка / богатство)
    award_achievement(uid, "first_purchase")
    check_and_award_achievements(uid)
    await edit_section(
        query,
        caption=(
...
async def farmer_buy_confirm(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    award_achievement(uid, "first_farmer")
    check_and_award_achievements(uid)
...
async def admin_actions(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "admin_reset_autumn":
        ...
        return
    # Удалено: admin_clans и связанные действия
    await edit_section(query, caption="❓ Неизвестная команда.", image_key="admin")


# Удалено: весь функционал кланов
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ------------------- Техподдержка -------------------
    if data == "support":
        await support_section(query, context)
        return
    # ------------------- Промокоды -------------------
    if data == "promo":
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get("awaiting_support_message"):
        ...
        content = txt.strip()
        if not content:
            await update.message.reply_text("❌ Пустое сообщение. Опишите проблему.")
            return
        log_user_action(uid, f"Техподдержка: {content}")
        try:
            await context.bot.send_message(CHAT_ID, f"🆘 ТП от {uid}: {content}")
        except Exception:
            pass
        context.user_data["awaiting_support_message"] = False
        await update.message.reply_text("✅ Сообщение отправлено в техподдержку. Мы свяжемся с вами.")
        return
...
def main() -> None:
    ...
    # Удалено: проверка клановых битв
