...
async def status_section(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    cur.execute("SELECT COUNT(*) as count FROM users WHERE last_activity > ?", (hour_ago,))
    users_last_hour = cur.fetchone()["count"]
    
    cur.execute("SELECT COUNT(*) as count FROM users WHERE last_activity > ?", (day_ago,))
    users_last_24h = cur.fetchone()["count"]
    ...
    text = (
        ...
        f"🕐 За последний час: {users_last_hour}\n"
        f"📅 За последние 24 часа: {users_last_24h}\n"
        f"━━━━━━━━━━━━━━━━━━━━"
    )
